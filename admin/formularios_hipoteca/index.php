<?php 
 ...
 //Escribir el Query
 $query = "SELECT * FROM formularios_hipoteca"; 
 ...
 $reultadoConsulta = mysqli_query($db, $query);
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if($id){
             ...
             //eliminar proyecto
            $query = "DELETE FROM formularios_hipoteca WHERE cod_formH = ${id}";

            $resultado = mysqli_query($db, $query);

            if($resultado) {
                header('location: \admin\formularios_hipoteca\index?resultado=3');
            }
        }
...
<main class="contenedor seccion">
    ...
    <?php if(intval($resultado)  === 1): ?>
        ...
        <br>
        <a href="/pdf-fiador.php" class="btn btn-primary disabled"><b>PDF</b> </a>
		</div>
		<br>
...
<br>
    <table  class= "table table-bordered table-striped table-hover table_id" id="tblDatos">  
        ...
        <tbody> <!--mostrar los resultados-->
            <?php  while($formularioH = mysqli_fetch_assoc($reultadoConsulta)): ?>
            <tr>
                
                <td><?php echo $formularioH['monto_solicitar']; ?></td>
                <td><?php echo $formularioH['nombre_cliente']; ?></td>
                <td><?php echo $formularioH['edad']; ?></td>
                <td> <?php echo $formularioH['cedula']; ?></td>
                <td> <?php echo $formularioH['canton']; ?></td>
                <td> <?php echo $formularioH['nombre_emp']; ?></td>
                <td> <?php echo $formularioH['numero_finca']; ?></td>
                <td>

                <form method="POST" class="w-100">

                <input type="hidden" name="cod_proyecto" value="<?php echo $formularioH['cod_formF'];?>">
                ...
                <a href="actualizar.php?cod_formH=<?php echo $formularioH['cod_formH'];?>" 
